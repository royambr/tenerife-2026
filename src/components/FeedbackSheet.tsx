...
export function FeedbackSheet({ open, onClose, defaultScreen }:{
  ...
  const [who, setWho] = useState(currentId);
  ...
  return (
    <Sheet open={open} onClose={onClose} title="✍️ הפידבק שלך">
      <div className="space-y-4">
        ...
        {/* Who */}
        <div>
          <div className="text-[12px] font-bold text-ocean-700 mb-1.5">מי אתה?</div>
          <div className="flex flex-wrap gap-1.5">
            {participants.map(p => (
              <button key={p.id} type="button" onClick={() => setWho(p.id)}
                className={`min-h-[36px] rounded-full px-3 py-1.5 text-[12px] font-semibold transition ${
                  who === p.id ? 'bg-ocean-700 text-white' : 'bg-ocean-50 text-ocean-700 hover:bg-ocean-100'
                }`}>
                {p.emoji} {p.name}
              </button>
            ))}
...
export function FeedbackFab({ activeTab }:{ activeTab?: string }) {
  ...
  return (
    <>
      <button type="button" onClick={handleOpen} aria-label="שלח פידבק"
        className={`fixed right-4 z-[90] w-12 h-12 rounded-full bg-sunset-500 text-white text-2xl shadow-2xl
                    flex items-center justify-center bottom-[96px] lg:bottom-6
